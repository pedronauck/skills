...
8. Read `references/checklist.md` and ensure every required category has a planned validation.
...
12. When a Web UI surface exists, read `references/web-ui-qa.md` and select 3-5 critical user flows to exercise through the browser. Prioritize flows that cover the changed surface and the most business-critical paths.
...
5. Compare checked boxes and `declared_status` against evidence. A checked item or `status: completed` without code or runtime proof is a QA failure.
